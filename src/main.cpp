...
arma::mat sample_eta_all(arma::mat Y, arma::mat X, arma::mat xi, arma::mat nu, arma::mat Lambda, 
  ...
  */ 
  ...
  arma::vec sigsq_D_inv = join_cols(pow(sigsq_y, -1.0), pow(sigsq_x, -1.0));
  ...
  // Get (common) covariance matrix used when sampling each individual \eta_i.
  ...
  for( int i=0; i<N; i++ ){
    ...
    int P_obs = sum(obs_Y_i) + num_feats;
  ...
  */
  ...
  for( int h=0; h<K; h++ ){
    ...
    if(h==0){
      ...
      shape = a2 + (K-h)*(D+S)/2; 
      rate = 1.0/delta_ome(h-1); // Evan's modification
    }
 ...
 * 
 * For use in examples and display purposes.
 * 
 */
